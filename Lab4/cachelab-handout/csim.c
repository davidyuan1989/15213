...
/*Location the set according to the address*/
...
/*Create a Catch, which contains a set of sets*/
Set* construction(){
	int numSets = 1 << s;
	int maxLines = E;
...
void search(Set *headSet, unsigned int address){
	...
	Line *preLine = NULL;
	/*Check whether have exsiting tag*/
	while(curLine != NULL){
		if(curLine -> tag == tag){
			numHits++;
			if(preLine != NULL){
